...
## 90-Second Video Script (Version B — Glass Cam)

**Learning Outcome:** "Data collection is configuration, not code — and resilience is built in."

**Demo Setup:** Vino wine-pouring robot with a glass held in one gripper. Glass cam mounted on the front right corner of the wine-pouring cart captures images for building a full-vs-not-full classifier training dataset. Ethernet cable for network disconnect demo.
...
- Show glass cam feed (front right corner of cart)
- Show data capture config in Viam app — one camera, frequency, method

*Presenter:*
"This is a wine-pouring robot. It needs to know whether a glass is full or not. That means a classifier, and a classifier needs training data. This camera on the cart watches the glass. One config block — capture frequency, capture method. That's it."
...
*Visual:*
- Cloud dashboard: images from the glass cam appearing — glass in gripper from cart-level angle
...
*Presenter (voiceover):*
"If I wanted to filter out frames where the glass isn't visible — maybe the arm is moving — I'd train a simple model on a small number of images and use it to filter at the edge. Unusable frames never leave the robot."
...
*Visual:*
- Cloud dashboard with data flowing from glass cam
- Disconnect network — visible action (unplug ethernet or toggle WiFi)
- Dashboard stops updating
- Beat. Let the viewer sit with it.
- Reconnect network
- Batch of images appears in the dashboard

*Presenter (voiceover):*
"Disconnect the network. Data keeps capturing locally. Reconnect. Everything syncs. No retry code, no queue management, no error handling. All from configuration."
...
- Payoff is short and direct. No taglines. Production capture line lands the long-term value.

**The narrative arc:**
Data collection infrastructure is the real project, not the capture itself (hook) → This robot needs a full-vs-not-full classifier (setup) → Configuration, data flows (demo) → Edge filtering is possible too (mention) → Network goes down, data survives (resilience) → All configuration, no code, stays on in production (payoff)
...
5. Data capture isn't just for initial training — it stays on in production with configuration-controlled sync
...
- Production capture with selective sync: keep capturing in production, sync only on key events (e.g., failures) to continuously improve models

---

## B-Roll Needed

- Vino robot on bench with glass in gripper
- Glass cam mounted on front right corner of cart
- Glass cam feed showing glass from cart-level angle
- Close-up of ethernet cable being unplugged and plugged back in
- Presenter with robot in background

## Screen Recordings Needed

- Viam app: data capture config for glass cam (frequency, method)
- Cloud dashboard: image thumbnails appearing from glass cam
...
- Camera label if needed ("Glass Cam")
