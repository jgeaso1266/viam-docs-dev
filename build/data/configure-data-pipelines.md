...
**Time:** ~25 minutes
**Prerequisites:** [Capture and Sync Data](../foundation/capture-and-sync-data.md) (data flowing to the cloud) | [Query Data](query-data.md) (familiarity with MQL)
**Works with:** Simulation ✓ | Real Hardware ✓

## What You'll Learn

- What data pipelines are and how they transform raw sensor data into precomputed summaries
- How to create a pipeline using the CLI, Python, and Go
- How cron schedules determine the query time range for each pipeline run
- How to query pipeline results from the pipeline sink data source
- How to list, enable, disable, and delete pipelines
- How to monitor pipeline execution history and diagnose failures
- How to configure the hot data store for fast queries against recent data
...
temperature per location over the last hour?" or "how many readings did each
machine produce today?", scanning every raw reading is slow and expensive.
...
document. Pipelines run on a cron schedule, and each run automatically scopes
its query to the time window since the last run.

The hot data store complements pipelines by keeping a rolling window of recent
raw data in a fast-access database. If you need both precomputed summaries
(pipelines) and quick access to the last N hours of raw readings (hot data
store), you can use both together.
...
For example, instead of querying 3,600 raw temperature readings to compute the
average for the last hour, a pipeline runs that aggregation once per hour and
stores the result. Your dashboard then reads a single document instead of
scanning thousands.
...
- Familiarity with MQL aggregation pipelines (from
  [Query Data](query-data.md))
- The [Viam CLI](https://docs.viam.com/cli/) installed (for CLI-based pipeline
  creation)
- An API key with **Owner** permissions on the organization (only org owners can
  create pipelines)
...
### 3. Understand the schedule format {#schedule-format}
...
When it runs at 3:00 PM, it queries data from 2:00 PM to 3:00 PM. When it runs
at 4:00 PM, it queries data from 3:00 PM to 4:00 PM. Each run processes exactly
one window of data with no gaps and no overlaps.

Choose a schedule that matches how frequently you need updated summaries. Shorter
intervals produce more granular summaries but create more pipeline sink documents.
Longer intervals produce fewer documents but with longer delays before summaries
are available.
...
In the example above, each document has `location`, `avg_temp`, and `count`
fields.
...
retains. Data older than this window is automatically removed. Choose a window
that covers your typical query range -- 24 hours for daily dashboards, 1-2 hours
...
- [Query Data](query-data.md) -- learn more MQL patterns to use in your
  pipeline aggregation queries.
- [Filter at the Edge](filter-at-the-edge.md) -- reduce the volume of raw data
  before it reaches the cloud, making your pipelines faster and cheaper.
- [Visualize Data](visualize-data.md) -- build dashboards on top of your
  precomputed pipeline summaries.
