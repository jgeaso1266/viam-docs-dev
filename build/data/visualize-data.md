...
**Time:** ~20 minutes
**Prerequisites:** [Capture and Sync Data](../foundation/capture-and-sync-data.md) (data flowing to the cloud) | [Query Data](query-data.md) (for programmatic visualization)
**Works with:** Simulation ✓ | Real Hardware ✓

## What You'll Learn

- How to create a Teleop dashboard in the Viam app with time-series and camera widgets
- How to configure database credentials for third-party visualization tools using the Viam CLI
- How to connect Grafana to your Viam data and build a monitoring dashboard
- How to connect other third-party tools (Tableau, Looker Studio) using the same credentials
- How to build a programmatic dashboard in Python and Go using the Viam SDK and matplotlib
...
Viam gives you multiple ways to visualize data. The built-in Teleop dashboard
is the fastest path -- no external tools, no credentials to configure, just
widgets on a canvas. When you need richer visualization (custom queries,
cross-machine dashboards, alerting), third-party tools like Grafana connect
directly to the same data store. And when you need full control -- embedding
charts in your own application, generating PDF reports, feeding data into a
notebook -- the Python and Go SDKs let you query data and pipe it into any
visualization library you choose.

This block covers all four approaches, from simplest to most flexible.
...
options, each suited to a different use case:
...
access. The tradeoff is setup time: you need to configure database credentials
...
charts, Plotly dashboards, HTML reports, or any other format. This is the right
choice when you need to embed visualizations in your own application or generate
automated reports.

## Components Needed

- An organization with captured tabular data (from
  [Capture and Sync Data](../foundation/capture-and-sync-data.md))
- For Teleop: a machine with at least one sensor, camera, or other component
  that captures data
- For Grafana: a Grafana instance (local Enterprise or Cloud free trial) and the
  [Viam CLI](https://docs.viam.com/dev/tools/cli/) installed
- For programmatic visualization: the Viam Python SDK or Go SDK installed

## Steps

### 1. Build a Teleop dashboard (Viam's built-in)

The Teleop dashboard is the fastest way to visualize data from a single machine.
No database credentials, no external tools, no code.
...
using the Viam CLI. This gives the external tool read access to your synced
data.
...
3. Find your organization ID:
   ...
   Copy the ID of the organization whose data you want to visualize.

4. Configure database credentials. This creates a database user for your
   organization and sets a password:

   ```bash
   viam data database configure --org-id=<YOUR-ORG-ID> --password=<NEW-PASSWORD>
   ```

   Replace `<YOUR-ORG-ID>` with your organization ID and `<NEW-PASSWORD>` with a
   strong password. Store this password securely -- you will need it when
   connecting your visualization tool.
...
5. Get the database hostname:
   ...
   This returns a hostname and a connection URI. Copy both -- you will use them
   in the next step.

   The output looks like:

   ```
   Hostname: data-federation-abcdef12-abcd-abcd-abcd-abcdef123456-e4irv.a.query.mongodb.net
   Connection URI: mongodb://data-federation-abcdef12-abcd-abcd-abcd-abcdef123456-e4irv.a.query.mongodb.net
   ```
...
Grafana is the most popular open-source visualization tool, and it works well
with Viam's MongoDB-backed data store. This step walks through the full setup.

#### Install Grafana

If you do not already have a Grafana instance, choose one of these options:
...
> **Important:** Install the MongoDB **data source**, not the MongoDB
> **integration**. They are different plugins. The data source is what allows
> you to query MongoDB from Grafana dashboards.
...
2. Fill in the following fields:
   ...
   - **Connection string**:
     ...
     Replace `<HOSTNAME>` with the hostname from step 2. The database name
     `sensorData` is the default for synced sensor data.
...
The `${__from}` variable is a Grafana global variable. It is populated
automatically from the time range selector at the top of your dashboard. When
you change the time range, the query re-runs with the new value. See [Grafana's
global variables documentation](https://grafana.com/docs/grafana/latest/dashboards/variables/add-template-variables/#global-variables)
for the full list.
...
3. Build dashboards using the tool's native interface. Query the `readings`
   collection in the `sensorData` database using SQL or MQL, depending on what
   the tool supports.

> **Tip:** Before building dashboards, use [MongoDB Compass](https://www.mongodb.com/products/tools/compass)
...
### 5. Build a programmatic dashboard (Python / Go)
...
file. You can adapt this pattern for any visualization library -- Plotly for
interactive charts, Bokeh for web dashboards, or any other Python plotting tool.
...
This generates a PNG chart of your sensor data. The `gonum.org/v1/plot` library
handles the rendering. For more advanced visualizations in Go, consider
generating data as JSON and rendering it in a web frontend.
...
1. **Build a Teleop dashboard.** Create a workspace, add a time-series widget
   ...
   time. Change a condition near your sensor (warm it up, cover it, move
   something in front of the camera) and confirm the dashboard reflects the
   change.

2. **Connect Grafana.** Run the CLI commands to configure database access, set
   up the Grafana MongoDB data source, and run the example MQL query. You
   should see sensor readings in the query results. Create a time-series panel
   and verify the chart updates when you change the time range selector.
...
3. **Run the Python or Go script.** Copy one of the programmatic examples, fill
   ...
   file in your working directory with a plotted line chart of your sensor data.

## Troubleshooting

### Teleop dashboard shows no data
...
- [Query Data](query-data.md) -- learn the full range of SQL and MQL queries
  you can use to extract insights from your data.
- [Configure Data Pipelines](configure-data-pipelines.md) -- set up aggregated
  views and derived metrics to power your dashboards more efficiently.
- [Filter at the Edge](filter-at-the-edge.md) -- reduce noise in your
