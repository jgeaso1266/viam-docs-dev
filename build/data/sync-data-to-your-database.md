...
**Time:** ~25 minutes
**Prerequisites:** [Capture and Sync Data](../foundation/capture-and-sync-data.md) (data flowing to the cloud) | [Query Data](query-data.md) (familiarity with MQL queries)
**Works with:** Simulation ✓ | Real Hardware ✓

## What You'll Learn

- Why you would use Viam as an ingestion layer while owning your own data storage
- How to configure database query access credentials with the Viam CLI
- How to connect to your Viam data from external tools using a MongoDB connection URI
- How to pull data programmatically with the Python and Go SDKs and write it to your own database
- How to build a sync script that periodically moves new data from Viam to your database
- How to export data in bulk with the Viam CLI
...
This block treats Viam as an ingestion layer. Viam handles the hard part --
reliable device-to-cloud transport from machines that go offline, lose
connectivity, or restart unexpectedly. Your database handles the part you
already know how to do -- storage, retention, access control, and downstream
analytics. You get the best of both: Viam's edge-to-cloud reliability and your
own infrastructure as the system of record.

You will set up database query credentials, connect to your Viam data directly,
pull data programmatically, and build a repeatable sync pattern that keeps your
database current.
...
| Webhooks | Viam POSTs to your endpoint | Near-real-time | Alerts, event-driven workflows |

This block focuses on the first three patterns. Webhooks are covered in
[Trigger on Detection](../stationary-vision/trigger-on-detection.md).

### Viam as ingestion layer

In this architecture, Viam handles everything from the machine to the cloud:
data capture on the device, local buffering when connectivity drops, and
...
- You do not need to build edge-to-cloud transport infrastructure.
- You do not need to handle offline buffering, retry logic, or deduplication.
- You keep full control of your data once it reaches your systems.

## Components Needed

- An organization with captured tabular data (from
  [Capture and Sync Data](../foundation/capture-and-sync-data.md))
- The [Viam CLI](https://docs.viam.com/cli/) installed and authenticated
- A destination MongoDB cluster (MongoDB Atlas, self-hosted, or any
  MongoDB-compatible database) -- needed for steps 4 and 5

If you do not have a destination database yet, you can still follow steps 1
through 3 to configure access and explore your data.
...
4. Retrieve the connection hostname:
   ...
   The output includes the hostname and a connection URI:

   ```
   MongoDB Atlas Data Federation instance hostname: data-federation-abcd1e2f-...
   MongoDB Atlas Data Federation instance connection URI: mongodb://db-user-abcd1e2f-...:YOUR-PASSWORD@data-federation-abcd1e2f-.../?ssl=true&authSource=admin
   ```

   Save both values. You will use the connection URI in the next step and in
   your scripts.
...
5. Query a few documents to verify:
   ...
   You should see your most recent captured readings with fields like
   `time_received`, `component_name`, `component_type`, and `data`.
...
commands. This is the simplest way to get a copy of your data onto your local
filesystem.
...
Both commands create `data/` and `metadata/` directories inside your
destination folder. Files are downloaded in parallel, so the order they appear
is not guaranteed.
...
### 4. Pull data programmatically (Python / Go)

For automated workflows, use the Viam SDK to query data from Viam and write it
directly to your own MongoDB cluster. This gives you full control over what data
you pull, how you transform it, and where you store it.
...
`YOUR-ORGANIZATION-ID`, and the MongoDB URI) with your actual credentials.
Find your organization ID in the Viam app under **Settings** in the left
navigation.

### 5. Build an ongoing sync script

A one-time pull is useful, but most teams need ongoing sync. The key is tracking
the last sync timestamp so each run only pulls new data. This avoids
re-processing data you have already inserted and keeps query costs low.
...
- **Every 5 minutes** works well for operational dashboards that need
  near-current data.
- **Every 15 minutes** balances freshness with query cost for most use cases.
- **Every hour** is sufficient for long-term analytics and compliance archives.
- **High-frequency polling (every minute or less) compounds query costs at
  scale.** If you need near-real-time delivery, use webhooks instead of polling.
...
- If filtering by time, make sure the time range covers a period when data was
  actually captured. Use `$gte` with a date far in the past to verify data
  exists before narrowing the window.
...
- Check that the API key has data access permissions. API keys can be scoped to
  specific resources.

### Duplicate records in destination database

- The sync scripts in this block do not deduplicate by default. If a run is
...
- [Configure Data Pipelines](configure-data-pipelines.md) -- set up windowed
  roll-ups and derived metrics to reduce data volume before syncing.
- [Visualize Data](visualize-data.md) -- connect Grafana or other dashboard
  tools to your Viam data using the MongoDB connection URI from this block.
- [Query Data](query-data.md) -- write advanced SQL and MQL queries against your
  data in Viam's cloud before pulling it to your own systems.
