...
**Time:** ~20 minutes
**Prerequisites:** [Capture and Sync Data](../foundation/capture-and-sync-data.md) (data capture and cloud sync working)
**Works with:** Simulation ✓ | Real Hardware ✓

## What You'll Learn

- Why filtering data before it leaves the machine saves bandwidth and storage
- How to reduce capture frequency for simple time-based sampling
- How to configure conditional sync so only relevant data reaches the cloud
- How to build a filtered camera module that captures only when meaningful change occurs
- How to apply threshold-based filtering for sensor data
- How to manage local storage on constrained devices
...
uninteresting data before it ever leaves the device. This is especially
important for machines on cellular connections, metered networks, or devices with
limited local storage.

This block covers four techniques, from simplest to most powerful:
...
- A machine running `viam-server` with data capture configured (from
  [Capture and Sync Data](../foundation/capture-and-sync-data.md))
- At least one component to capture from (camera, sensor, or any configured
  component)
...
> **Tip:** Start with the lowest frequency that meets your needs. You can always
...
> **Note:** Conditional sync still captures data locally at your configured
> frequency. It only controls when that data gets uploaded. Make sure your
> device has enough local storage for the capture buffer (see
> [Storage management](#6-manage-local-storage) below).

### 3. Build a filtered camera module (Python)
...
[Deploy a Module](../development/deploy-a-module.md) for full deployment
...
### 4. Build a filtered camera module (Go)

Here is the same concept implemented in Go. This is useful if your machine
runs on a resource-constrained device where Go's lower memory footprint is
advantageous.
...
Build and configure it the same way as the Python version, using the same
attributes:
...
### 5. Filter sensor data by threshold
...
On constrained devices (Raspberry Pi, Jetson Nano, single-board computers),
...
3. Compare data volume before and after filtering:
   ...
   - Compare to the rate you saw in
     [Capture and Sync Data](../foundation/capture-and-sync-data.md) before
     filtering.
...
- [Query Data](query-data.md) -- write SQL and MQL queries against your
  filtered, high-signal dataset.
- [Create a Dataset](../train/create-a-dataset.md) -- use filtered captures to
  build cleaner training datasets for ML models.
- [Write a Module](../development/write-a-module.md) -- learn more about
