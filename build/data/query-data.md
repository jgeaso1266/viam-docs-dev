...
**Time:** ~20 minutes
**Prerequisites:** [Capture and Sync Data](../foundation/capture-and-sync-data.md) (data flowing to the cloud)
**Works with:** Simulation ✓ | Real Hardware ✓

## What You'll Learn

- The schema of Viam's `readings` table and how captured data is structured
- How to write SQL queries that filter, sort, and extract fields from captured data
- How to write MQL aggregation pipelines for grouping, counting, and computing statistics
- How to navigate the nested JSON in the `data` column
- How to query data programmatically from Python and Go
- How to export query results for use in external tools

## What Problem This Solves

Captured data is only useful if you can ask questions of it. The Capture and Sync
Data block got data flowing into the cloud. Now you need to answer questions
like "what were the last 10 readings from my sensor?", "how many detections
happened per hour today?", or "what was the average temperature over the last
week?" Viam provides both SQL and MQL query interfaces -- SQL for familiar
relational-style queries and MQL for MongoDB-style aggregation pipelines that
handle complex grouping and nested data more naturally. You can run queries in
the Viam app or programmatically from your own code.
...
If you don't have captured data yet, you can still follow along with the query
patterns below. The queries will return empty results, but the syntax and
structure are the same regardless of what data is stored.
...
### 5. Query data programmatically with Python
...
### 6. Query data programmatically with Go
...
### 7. Export data for external tools
...
Use the query scripts from steps 5 or 6 to fetch data and write it to a file.
...
### Programmatic query fails with authentication error
...
- [Create a Dataset](../train/create-a-dataset.md) -- organize queried data into
  datasets for ML training.
- [Add Computer Vision](../vision-detection/add-computer-vision.md) -- capture
  detection results that you can query with the patterns above.
- [Trigger on Detection](../stationary-vision/trigger-on-detection.md) -- use
