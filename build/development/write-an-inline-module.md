...
**Time:** ~20 minutes
**Prerequisites:** [Connect to Cloud](../foundation/connect-to-cloud.md) (machine created, `viam-server` running, machine online)
**Works with:** Simulation ✓ | Real Hardware ✓

## What You'll Learn

- What a module is and how modules extend Viam's capabilities
- What an inline module is and how it differs from a separate-process module
- When to use the Generic component and its DoCommand pattern
- How to generate, implement, and test a custom inline module in Python or Go
...
This block walks you through generating a module scaffold, implementing control
logic using the Generic component's DoCommand pattern, and testing the module
...
module-provided resources exactly the same way you interact with built-in ones --
through the Viam app, the SDKs, or the API.

Modules can provide components (cameras, sensors, motors, etc.), services (ML
model inference, SLAM, navigation), or both.
...
that accepts an arbitrary JSON command and returns a JSON response. This is
flexible enough to handle any custom logic.
...
the same format. You define your own command structure -- there is no fixed
schema. A common pattern is to include an `action` key that selects what the
component should do:
...
This pattern gives you full control over the interface without needing to
implement a full Viam resource API.
...
The Viam CLI can generate a complete module project with the correct file
structure, boilerplate, and metadata. Run the generator and follow the prompts:
...
After generation completes, you have a directory with all the files you need.

### 2. Understand the generated files

The generator creates a standard project structure. The exact files depend on
...
- `requirements.txt` lists Python packages your module needs.
...
The `validate_config` method (Python) or `Validate` method (Go) runs every time
the machine configuration changes. It checks that the attributes passed to your
component are valid and declares dependencies on other components.

This example validates that a `board_name` and `pin` attribute are present. The
component depends on a board to toggle a GPIO pin.
...
1. **Required dependencies** -- component names that must exist and be ready
   before your component starts. Here, the board must be available because the
   component needs it to control a GPIO pin.
...
create the component. This prevents your module from starting in a broken state.
...
In Go, the `Validate` method returns a slice of dependency names and an error.
The config struct fields are populated automatically from the JSON attributes
using struct tags.
...
`DoCommand` is the core entry point for your component's logic. Every call from
the Viam app's DoCommand panel, the SDKs, or the API goes through this method.

This example implements a toggle switch. Sending `{"action": "toggle"}` flips a
GPIO pin on or off. Sending `{"action": "get_status"}` returns the current
state.
...
would be a better fit. You can always migrate from Generic to a typed API later.

### 6. Test locally

Run the module on the same machine as `viam-server` and test it through the Viam
app.
...
If you have a board connected, you can verify the GPIO pin state with a
multimeter or an LED connected to the pin.
...
If all four checks pass, your inline module is working correctly.
...
- For Python modules, confirm that the Viam SDK is installed in the Python
  environment that `viam-server` uses. Run
  `pip install viam-sdk` on the machine.
...
- Make sure dependency names (returned from `validate_config`) match the names of
  actual components on the machine. If you declare a dependency on `my-board`,
  a component named `my-board` must exist.

### Module crashes when accessing dependencies
...
- [Write a Module](write-a-module.md) -- build a full separate-process module
  with a typed resource API for production use.
- [Deploy a Module](deploy-a-module.md) -- package and upload your module to the
  Viam registry so other machines can use it.
- [Filter at the Edge](../data/filter-at-the-edge.md) -- use modules to
