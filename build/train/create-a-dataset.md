...
**Time:** ~20 minutes
**Prerequisites:** [Capture and Sync Data](../foundation/capture-and-sync-data.md) (images in the cloud)
**Works with:** Simulation ✓ | Real Hardware ✓

## What You'll Learn

- What a dataset is and why you need one before training an ML model
- The minimum requirements for a useful training dataset
- How to create a dataset and add images using the web UI, CLI, and SDK
- How to tag images for classification tasks
- How to draw bounding boxes for object detection tasks
- How to verify your dataset meets quality requirements before training

## What Problem This Solves

You have images flowing into Viam's cloud from your machine's camera. Raw images
are not enough to train an ML model. You need a curated, labeled collection --
a dataset -- that teaches the model what to look for. Without a dataset, there is
nothing to train on.

Building a good dataset is the most important step in the ML pipeline. A model
is only as good as the data it learns from. If your dataset is small, poorly
labeled, or unrepresentative of real conditions, your model will perform poorly
in production regardless of how sophisticated the training algorithm is.

This block walks you through creating a dataset, populating it with images,
labeling those images for either classification or object detection, and
verifying that the dataset meets the requirements for training.
...
- An organization with captured images in the cloud (from
  [Capture and Sync Data](../foundation/capture-and-sync-data.md))
- Images that represent the categories you want your model to recognize

If you do not have captured images yet, you can still create an empty dataset
...
async def main():
    ...
    # Tag specific images by their binary IDs
    binary_ids = [
        BinaryID(
            file_id="file-id-1",
            organization_id=ORG_ID,
...
async def main():
    ...
    # Add a bounding box to a specific image
...
- [Train a Model](train-a-model.md) -- use your labeled dataset to train a
  classification or object detection model.
- [Add Computer Vision](../vision-detection/add-computer-vision.md) -- deploy a
  trained model to your machine's camera feed.
- [Query Data](../data/query-data.md) -- write queries to explore and analyze
