...
**Time:** ~20 minutes
**Prerequisites:** [Detect Objects (2D)](detect-objects-2d.md) (detections working)
**Works with:** Simulation ✓ | Real Hardware ✓

## What You'll Learn

- Why per-frame detections are insufficient for tracking and what persistence means
- How to match detections between consecutive frames using Intersection over Union (IoU)
- How to assign and maintain stable track IDs for detected objects
- How to detect when objects enter or leave the scene
- How to count entries and exits (e.g., people passing through a doorway)

## What Problem This Solves

Detection gives you a snapshot: in this frame, there are three people. In the next frame, there are still three people. But are they the same three people? Detection alone cannot answer that question. Each frame's detections are independent -- the model has no memory.

Tracking solves this by matching detections across consecutive frames. If a bounding box in frame N overlaps significantly with a bounding box in frame N+1, they are probably the same object. By assigning a stable ID to each tracked object, you can answer questions like: how long has this person been standing here? How many cars have passed through this intersection? Did the package move since the last check?

This is a code-level pattern, not a built-in Viam service. You build tracking logic on top of the detection API.
...
- A machine with a configured camera and vision service returning detections (from [Detect Objects (2D)](detect-objects-2d.md))
...
### Too many new tracks for the same object
...
- [Localize Objects in 3D](localize-objects-in-3d.md) -- combine tracking with depth data to track objects in 3D space.
- [Count Objects](../stationary-vision/count-objects.md) -- use tracking counts to build monitoring dashboards.
- [Trigger on Detection](../stationary-vision/trigger-on-detection.md) -- trigger automated actions when tracked objects enter or exit a scene.
