...
- What classifications are and how they differ from detections
- The difference between single-label and multi-label classification
- How to use `GetClassifications` and `GetClassificationsFromCamera`
- How the `count` parameter controls how many top results are returned
- When to use classification vs detection for your application

## What Problem This Solves

Detection tells you where objects are in an image. Classification tells you what the entire image (or a region of it) contains. If you need to answer "is this a picture of a cat or a dog?" rather than "where are the cats and dogs in this picture?", classification is the right tool. It is also simpler: the result is a label and a confidence score, with no bounding box coordinates to manage.

Classification is useful for sorting, monitoring, and triggering. Is the conveyor belt carrying the right part? Is the room occupied or empty? Is the plant healthy or diseased? These are classification questions. This block shows you how to get classifications from your vision service and use them in application logic.
...
- A model that supports classification (many models support both detection and classification)
...
- [Detect Objects (2D)](detect-objects-2d.md) -- get per-object bounding boxes instead of whole-image labels.
- [Track Objects Across Frames](track-objects-across-frames.md) -- maintain object identities across consecutive frames.
- [Trigger on Detection](../stationary-vision/trigger-on-detection.md) -- use classification or detection results to trigger automated actions.
