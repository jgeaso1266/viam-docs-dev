...
**Time:** ~20 minutes
**Prerequisites:** [Add a Camera](../foundation/add-a-camera.md) (depth camera configured)
**Works with:** Simulation ✓ | Real Hardware ✓

## What You'll Learn

- What depth cameras provide and how they differ from standard RGB cameras
- What point clouds are and how they represent 3D space
- How to retrieve point cloud data using the camera API
- How to read depth at a specific pixel location
- How to combine depth data with 2D detections to measure distance to objects
- How camera intrinsic parameters affect 2D-to-3D projection

## What Problem This Solves

A standard camera gives you a flat 2D image. You can see that there is a box on the table, but you cannot tell whether the box is 30 centimeters away or 3 meters away. Depth cameras solve this by providing distance information for every pixel in the image, or by generating a 3D point cloud of the scene.

This depth information is essential for robotics tasks that involve physical interaction: picking up objects, navigating around obstacles, measuring the size of real-world objects, or understanding the layout of a room. This block shows you how to get depth data from your camera and extract useful distance measurements.
...
- A machine running `viam-server` with one of the following:
  - A depth camera (Intel RealSense, Oak-D, ZED, or similar)
  - A `fake` camera configured to simulate depth (for testing without hardware)
- The camera configured in Viam (from [Add a Camera](../foundation/add-a-camera.md))

If you do not have a depth camera, you can use the `fake` camera model. It generates simulated point cloud data that you can use to test your code.
...
- [Localize Objects in 3D](localize-objects-in-3d.md) -- project 2D detections into 3D space using the depth data from this block.
- [Detect Objects (2D)](detect-objects-2d.md) -- get 2D detections to combine with depth measurements.
- [Define Your Frame System](../motion/define-your-frame-system.md) -- understand how coordinate frames relate to each other when placing depth data in the context of your full robot.
