...
**Time:** ~20 minutes
**Prerequisites:** [Add a Camera](../foundation/add-a-camera.md) (camera configured) | [Train a Model](../train/train-a-model.md) (or use a pre-trained model)
**Works with:** Simulation ✓ | Real Hardware ✓

## What You'll Learn

- How Viam separates ML model loading from vision inference into two distinct services
- How to configure an ML model service to deploy a trained model to your machine
- How to configure a vision service that uses the ML model for detection or classification
- How to verify the pipeline works from the CONTROL tab before writing code
- What model formats Viam supports and how to reference them in configuration

## What Problem This Solves

You have a trained machine learning model and a camera, and you want your machine to understand what it sees. Getting from "I have a model file" to "my machine can identify objects in real time" requires loading the model, feeding it images, and interpreting the results. In Viam, this is handled by two cooperating services.

The **ML model service** loads your model file and makes it available for inference. The **vision service** connects the model to your camera, runs inference on each frame, and returns structured results -- bounding boxes for detection, labels for classification. By separating these concerns, you can swap models without changing your vision pipeline, or use the same model with multiple cameras.

This block configures both services so that downstream blocks -- Detect Objects, Classify Objects, Track Objects -- have a working vision pipeline to build on. Once this setup is complete, adding new vision capabilities is a matter of writing application code, not reconfiguring services.
...
The `tflite_cpu` implementation runs entirely on the CPU, so it works on any hardware -- no GPU required. For Raspberry Pi, Jetson Nano, and other edge devices, this is the standard approach. Performance depends on the model size: a MobileNet-based detector typically runs at 5-10 frames per second on a Raspberry Pi 4.
...
- A machine running `viam-server` with a configured camera (from [Add a Camera](../foundation/add-a-camera.md))
- A trained ML model, either:
  - Deployed from the Viam registry (from [Train a Model](../train/train-a-model.md))
  - A local `.tflite`, `.onnx`, or other supported model file on the machine

If you do not have a trained model yet, you can use a pre-trained model from the Viam registry. Several general-purpose object detection models are available, including models trained on the COCO dataset that detect common objects like people, cars, and animals. Pre-trained models are a good starting point for testing your pipeline before investing in custom model training.
...
If both tests pass, your vision pipeline is configured correctly and ready for use by downstream blocks.

## Troubleshooting

### Vision service fails to start
...
### ML model service fails to load the model
...
- On resource-constrained devices, large models may fail to load due to insufficient memory. Check system memory usage.

### Detections are empty even though the camera works
...
### Detections are inaccurate or noisy

- The model may need more training data. See [Train a Model](../train/train-a-model.md) for improving model quality.
...
### Multiple vision services on the same model
...
- [Detect Objects (2D)](detect-objects-2d.md) -- use the vision service to get bounding boxes, filter by confidence, and process detection results in code.
- [Classify Objects](classify-objects.md) -- use the vision service for whole-image classification instead of per-object detection.
- [Train a Model](../train/train-a-model.md) -- train a custom model on your own data for better accuracy on your specific use case.
