...
**Time:** ~25 minutes
**Prerequisites:** [Detect Objects (2D)](detect-objects-2d.md) | [Measure Depth](measure-depth.md) (detections + depth working)
**Works with:** Simulation ✓ | Real Hardware ✓

## What You'll Learn

- Why 2D detections are not enough for physical interaction and how 3D localization solves this
- How the `detector_3d_segmenter` projects 2D bounding boxes into 3D point clouds
- How to configure the 3D segmenter vision service
- How to use the `GetObjectPointClouds` API to retrieve 3D object data
- How noise filtering parameters (`mean_k`, `sigma`) affect results
- How the frame system connects camera-frame coordinates to robot-frame coordinates

## What Problem This Solves

A 2D detection tells you that there is a cup at pixel coordinates (300, 200)-(400, 350). But if your robot arm needs to pick up that cup, it needs to know the cup's position in 3D space: 0.5 meters ahead, 0.1 meters to the left, and 0.8 meters below the camera. Converting from pixel coordinates to real-world 3D coordinates requires depth data and camera calibration information.

The `detector_3d_segmenter` automates this conversion. It takes your existing 2D detector, combines its detections with depth data from your camera, and produces 3D point clouds for each detected object. Each point cloud includes the object's label and its 3D position relative to the camera. From there, the frame system can transform these positions into any other coordinate frame on your robot.
...
- A machine with a configured depth camera (from [Add a Camera](../foundation/add-a-camera.md) and [Measure Depth](measure-depth.md))
- A configured 2D detector vision service (from [Detect Objects (2D)](detect-objects-2d.md))
- The camera must provide depth data and intrinsic parameters
...
See [Define Your Frame System](../motion/define-your-frame-system.md) for how to set up these transforms.
...
### No objects returned even though 2D detections work

- Verify the depth camera is providing depth data. Run the depth measurement test from [Measure Depth](measure-depth.md) to confirm.
...
### Object positions are wildly inaccurate

- The camera's intrinsic parameters may be incorrect or missing. Check the camera configuration and verify that `fx`, `fy`, `ppx`, and `ppy` are set correctly. See [Measure Depth](measure-depth.md) step 7 for configuration details.
...
### Frame system transforms give unexpected results
...
- [Define Your Frame System](../motion/define-your-frame-system.md) -- set up coordinate frame transforms so 3D positions are usable by arms, bases, and other components.
- [Pick an Object](../manipulation/pick-an-object.md) -- use 3D object positions to plan and execute grasp motions with a robot arm.
- [Detect While Moving](../mobile-vision/detect-while-moving.md) -- run 3D localization while the robot is in motion for dynamic obstacle avoidance.
