...
**Time:** ~20 minutes
**Prerequisites:** [Connect to Cloud](../foundation/connect-to-cloud.md) (machine running viam-server)
**Works with:** Simulation ✓ | Real Hardware ✓

## What You'll Learn

- What the frame system is and why it matters for multi-component robots
- How to choose a world frame as your fixed reference point
- How to configure translation and orientation for each component's frame
- How parent-child frame hierarchies work
- How to add collision geometry to frames
- How to query and transform poses between frames programmatically

## What Problem This Solves

When your robot has multiple components -- an arm, a camera, a gripper, sensors --
each component reports positions in its own local coordinate system. A camera
sees an object at pixel (320, 240), but the arm needs to know the object's
position in millimeters relative to its own base. Without a unified spatial
model, you cannot translate between these coordinate systems.
...
- A machine with `viam-server` running (from [Connect to Cloud](../foundation/connect-to-cloud.md))
- At least one configured component (arm, camera, sensor, etc.)
- A tape measure or ruler for measuring physical offsets
...
component's physical size. See [Define Obstacles](define-obstacles.md) for
...
### Frames not appearing in the VISUALIZE tab
...
### Axes point the wrong direction
...
### Translation offsets look wrong in the visualizer
...
### TransformPose returns unexpected values
...
- [Configure Robot Kinematics](configure-robot-kinematics.md) -- set up the
  kinematic model that describes how your arm's joints create motion.
- [Calibrate Camera to Robot](calibrate-camera-to-robot.md) -- determine your
  camera's intrinsic parameters for accurate 2D-to-3D projection.
- [Define Obstacles](define-obstacles.md) -- add collision geometry so the
