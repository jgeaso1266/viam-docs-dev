...
**Time:** ~25 minutes
**Prerequisites:** [Define Your Frame System](define-your-frame-system.md) | [Add a Camera](../foundation/add-a-camera.md)
**Works with:** Real Hardware ✓ (calibration requires a physical camera)

## What You'll Learn

- What camera intrinsic parameters are and why they matter
- How distortion parameters correct for lens distortion
- The difference between eye-in-hand and eye-to-hand camera mounting
- How to capture calibration images using a chessboard pattern
- How to run a calibration script to compute intrinsic and distortion parameters
- How to add calibration parameters to your camera configuration
...
miss its targets. Calibration measures these parameters from a known pattern
(a chessboard) and produces the numbers your configuration needs.

Beyond intrinsics, the camera also needs a frame in the frame system that
describes its physical position and orientation relative to the robot. A camera
mounted on the arm (eye-in-hand) moves with the arm. A camera mounted on a
fixed structure (eye-to-hand) stays stationary while the arm moves. The frame
configuration differs for each case.
...
- A machine with a configured camera (from [Add a Camera](../foundation/add-a-camera.md))
- A printed chessboard calibration target (standard 8x6 or 9x7 inner corners)
- A flat, rigid surface to mount the chessboard (acrylic plate, cardboard, or
  similar)
- A computer with Python, NumPy, and OpenCV installed
...
### Calibration parameters look wrong (very large or very small values)
...
### Calibration accuracy varies with distance
...
- [Define Obstacles](define-obstacles.md) -- add collision geometry to your
...
- [Pick an Object](../arm-manipulation/pick-an-object.md) -- use calibrated
