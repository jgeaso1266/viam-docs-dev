...
**Time:** ~20 minutes
**Prerequisites:** [Define Your Frame System](define-your-frame-system.md) (frame system configured)
**Works with:** Simulation ✓ | Real Hardware ✓

## What You'll Learn

- Why obstacle definitions are essential for safe motion planning
- The three geometry types Viam supports: box, sphere, and capsule
- How to add static collision geometry directly to component frames
- How to define dynamic obstacles programmatically using WorldState
- How to pass obstacles to the motion service for collision-free path planning
- How to visualize obstacles in the Viam app
...
Obstacles come in two forms. Static obstacles are permanent fixtures like tables
and walls that you configure once and leave in place. Dynamic obstacles are
objects that change between motion calls -- a box that was just placed on the
table, or a keep-out zone that varies based on the task. Both types use the same
geometry primitives and integrate with the same motion planner.
...
- A machine with a configured arm and frame system (from
  [Define Your Frame System](define-your-frame-system.md))
- Knowledge of your workspace dimensions (tape measure recommended)
...
### WorldState obstacles are not working
...
- [Move to Pose](../arm-manipulation/move-to-pose.md) -- use the motion service
  to move the arm's end effector to a target position while avoiding obstacles.
- [Configure Robot Kinematics](configure-robot-kinematics.md) -- understand the
  kinematic model that the motion planner uses alongside obstacle definitions.
- [Calibrate Camera to Robot](calibrate-camera-to-robot.md) -- calibrate your
